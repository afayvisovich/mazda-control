...
**ОБНАРУЖЕНО:** Реальный сервер автомобиля работает на **`localhost:32960`** (Fake32960Server)

**Почему изменилось:**
- Изначально тестировался `172.16.2.30:50001` (AG35TspClient)
- Анализ логов показал, что штатное приложение использует `localhost:32960`
- Порт 32960 **РАБОТАЕТ** - все команды окон/спойлера идут через него
- Порт 50001 **НЕ РАБОТАЕТ** - постоянно `IOException`
...
| `TBoxSpoilerController.kt` | Контроллер для подключения к 127.0.0.1:32960 |
