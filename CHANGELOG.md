...
## v4.2 (2026-03-15) - Исправление сервера подключения

### Проблема
- ❌ Сервер `172.16.2.30:50001` (AG35TspClient) НЕДОСТУПЕН
- ❌ Системный AG35TspClient получает `IOException`
- ❌ Анализ логов показал: штатное приложение использует `localhost:32960`

### Решение
✅ **ИСПОЛЬЗУЕМ ПРАВИЛЬНЫЙ СЕРВЕР: `127.0.0.1:32960` (Fake32960Server)**

### Изменения
- 🎯 `TBoxSpoilerController`: сервер изменён на `127.0.0.1:32960`
- 🔍 `NetworkDiagnostics`: проверка порта 32960
- 📱 UI: кнопка диагностики обновлена
- 📄 `network_security_config.xml`: приоритет localhost

### Преимущества
- ✅ Порт 32960 РАБОТАЕТ (подтверждено логами)
- ✅ Все команды окон/спойлера идут через этот порт
- ✅ Не требует подключения к внешнему серверу
- ✅ Работает без интернета (локально)

### Документация
- 📄 `AG35TSPCLIENT_MIGRATION.md` - обновлено
- 📄 `NETWORK_DIAGNOSTICS.md` - обновлено
